...
agfires<-list()
for(i in 1:17){
  yearlyfires[[i]]<-molespdf90[molespdf90$YEAR==1999+i,]
...
# Binary # Convert data to binary
rc <- function(x) {
ifelse(x <=  1999, 0,
ifelse(x >  1999, 1, 0)) }
stackfirebin <- calc(stackfire, fun=rc)
plot(stackfirebin)

# Stack data
firefreq<-sum(!is.na(stackfire)) # Max for cell is 6 - highest return rate is 6 times over 16 years
plot(firefreq)

# Overlay selected sites
utmproj<-"+proj=utm +zone=30 ellps=WGS84" #spatial coordinate reference system in Serengeti
latlongproj<-("+proj=longlat +datum=WGS84")
rl<-read.csv("/Users/anotherswsmith/Documents/AfricanBioServices/Master projects/Joana Awuah Adofo/Coordinates.study.sites.csv")
names(rl)
rl_proj <-rl
coordinates(rl_proj)<- ~Longitude + Latitude #define which columns correspond to x's and y's
proj4string(rl_proj)<-latlongproj #define projection (originally defined above)
rl_projUTM<-spTransform(rl_proj,utmproj)

# Plot fires freq stacked and selected sites
plot(firefreq)
plot(rl_projUTM,add=T)

# Extract fire freq for each site
Fire.return<- extract(firefreq,mysp,sp=T, method='simple')
head(Fire.return)

#Fire.return<- data.frame(coordinates(rl_projUTM),
#                   rl_projUTM$Plot.Name, 
#                   extract(firefreq, rl_projUTM))
#names(Fire.return) <- c("x", "y", "Plot.Name", "value")

#library(xlsx)
write.xlsx(Fire.return, "/Users/anotherswsmith/Documents/AfricanBioServices/Master projects/Joana Awuah Adofo/Fire.return2.xlsx")
...
ifelse(x <=  1999, 0,
ifelse(x >  1999, 1, NA)) }
stackfirebinLATE <- calc(stacklatefire, fun=rc)
plot(stackfirebinLATE)

# Stack data
firefreqLATE<-sum(!is.na(stacklatefire)) # Max for cell is 6 - highest return rate is 6 times over 16 years
plot(firefreqLATE)
...
# Extract fire freq for each site
Fire.returnLATE<- extract(firefreqLATE,mysp,sp=T, method='simple')
head(Fire.returnLATE)
...
#library(xlsx)
write.xlsx(Fire.returnLATE, "/Users/anotherswsmith/Documents/AfricanBioServices/Master projects/Joana Awuah Adofo/Fire.return.LATE.xlsx")
